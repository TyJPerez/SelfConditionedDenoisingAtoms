...
def sph_harm_prefactor(k, m):
    return ((2 * k + 1) * math.factorial(k - abs(m)) /
            (4 * math.pi * math.factorial(k + abs(m))))**0.5
...
class angle_emb(torch.nn.Module):
    ...
    def forward_angle(self, angle):
        cbf = torch.stack([f(angle) for f in self.sph_funcs], dim=1)
        # angle += 3.14/2
        # cbf_2 = torch.stack([f(angle) for f in self.sph_funcs], dim=1)
        # cbf = torch.cat([cbf, cbf_2])
        return cbf
...
def xyz_to_dat(pos, edge_index, num_nodes, use_torsion = False):
    ...
    b = torch.cross(pos_ji, pos_jk, dim=-1).norm(dim=-1) #  fixed error
    angle = torch.atan2(b, a)

    # cross_vec = torch.cross(pos_ji, pos_jk)
    # zero_num = (cross_vec == 0).sum(axis=1)
    # sign = torch.ones_like(zero_num)
    # tmp_tensor = sign[zero_num == 0]
    # tmp_tensor[(cross_vec[zero_num==0] < 0).sum(axis=1) >= 2] = -1 # no zero: >=2 negative numbers
    # sign[zero_num == 0] = tmp_tensor
    # if (zero_num != 0).sum().item(): # has zero, same with the first non-zero element.
    #     contain_zero_item = cross_vec[zero_num!=0]
    #     sign_lst = []
    #     for item in contain_zero_item:
    #         sign_lst.append(torch.sign(item[item!=0][0]).item())
    #     sign[zero_num != 0] = torch.tensor(sign_lst, dtype=torch.long, device=sign.device)
    # angle = angle * sign
    if use_torsion:
        ...
        plane1 = torch.cross(pos_ji, pos_j0, dim=-1) # fixed error
        plane2 = torch.cross(pos_ji, pos_jk, dim=-1) # fixed error

        a = (plane1 * plane2).sum(dim=-1) # cos_angle * |plane1| * |plane2|
        # b = (torch.cross(plane1, plane2) * pos_ji).sum(dim=-1) / dist_ji 
        b = (torch.cross(plane1, plane2, dim=-1) * pos_ji).sum(dim=-1) / dist_ji # fixed error
